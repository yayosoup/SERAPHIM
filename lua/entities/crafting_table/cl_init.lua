...
function ENT:Draw()
    ...
    if LocalPlayer():GetPos():Distance( self:GetPos() ) < 500 then
        ...
        cam.Start3D2D( self:GetPos() + (self:GetAngles():Up() * -1.7) + (self:GetAngles():Up() * moveUpDistance2) + (self:GetAngles():Forward() * forwardDistance2), ang2, 0.1 )
            ...
            draw.SimpleText( "Chemicals: " .. self:GetChemical(), "SeraphimFinalsSub", 0, ( v.z / 3) - 10, NPC_COLOR, TEXT_ALIGN_CENTER)
            draw.SimpleText( "Tech Trash: " .. self:GetTechTrash(), "SeraphimFinalsSub", 0, ( v.z / 3) + 20, NPC_COLOR, TEXT_ALIGN_CENTER)
            draw.SimpleText( "Wood: " .. self:GetWood(), "SeraphimFinalsSub", 0, ( v.z / 3) + 50, NPC_COLOR, TEXT_ALIGN_CENTER)
            draw.SimpleText( "Metal: " .. self:GetMetal(), "SeraphimFinalsSub", 0, ( v.z / 3) + 80, NPC_COLOR, TEXT_ALIGN_CENTER)
            draw.SimpleText( "Scrap: " .. self:GetMetal(), "SeraphimFinalsSub", 0, ( v.z / 3) + 110, NPC_COLOR, TEXT_ALIGN_CENTER)
        cam.End3D2D()
    end
end
